...
- (void)viewDidLoad {
    ...
    if (isVertifySign) {         NSLog(@"验签结果==========>成功 \n");
    }else{
        NSLog(@"验签结果==========>失败 \n");
    }
    
    // 5.加密
    NSString *cipherText = [sm2Manager encryptWithPublickey:publicKey originalStr:str];
    NSLog(@"加密后的密文==========>%@ ",cipherText);
    
    // 6.解密
    NSString *originalStr = [sm2Manager decryptWithPrivateKey:priviteKey cipherText:cipherText];
    NSLog(@"解密后的原文==========>%@ \n",originalStr);
